...
// saveWholeLineup takes all lineups and saves to local storage
function saveWholeLineup(lineup) {
    localStorage.setItem("lineups", JSON.stringify(lineup));
}
...
export function paintLineup(data, parent) {
    ...
    function handleUndo() {
        ...
        // decrement each times played in roster for each player in current lineup
        // remove last lineup
        ...
        lineup.shift();
        // paint new lineup
        paintLineup(lineup, parent);
        // save the whole lineup
        saveWholeLineup(lineup);
        // save edited roster
        saveRoster(roster);
    }
